#!/usr/bin/env python3
"""
End-to-End Production Test Suite for Xavigate - Fixed Version
...
class E2ETestSuite:
    def __init__(self):
        self.headers = {}
        if AUTH_TOKEN:  # Add token if provided, regardless of ENV
    ...
    async def run_all_tests(self):
        ...
        print(f"🚀 Xavigate End-to-End Test Suite (Fixed)")
        print(f"{'=' * 70}")
    ...
    async def test_rag_search(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            return {"count": len(results), "results": results[:2] if results else []}
    ...
    async def test_session_memory_save(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            resp = await client.post(
                f"{STORAGE_URL}/api/memory/save",  # Fixed endpoint
                json=memory_data,
                headers=self.headers
            )
            resp.raise_for_status()
    ...
    async def test_chat_with_context(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if metadata.get('rag_hits') or 'career' in answer.lower():
                print(f"✅ RAG context appears to be used")
    ...
    async def test_session_memory_get(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            resp = await client.get(
                f"{STORAGE_URL}/api/memory/get/{self.session_id}",  # Fixed endpoint
                headers=self.headers
            )
            resp.raise_for_status()
    ...
    async def test_persistent_memory(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            resp = await client.get(
                f"{STORAGE_URL}/api/memory/persistent-memory/{self.user_id}",  # Fixed endpoint
                headers=self.headers
            )
    ...
    async def test_logging_system(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Query recent logs using the correct endpoint
            resp = await client.get(
                f"{STORAGE_URL}/api/logging/interactions/{self.user_id}",  # Fixed endpoint
                params={"limit": 10},
                headers=self.headers
            )
            ...
            if resp.status_code == 200:
                ...
                print(f"ℹ️ No logs found yet (status: {resp.status_code})")
    ...
    async def test_memory_stats(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            resp = await client.get(
                f"{STORAGE_URL}/api/memory/memory-stats/{self.user_id}",  # Fixed endpoint
                headers=self.headers
            )
            resp.raise_for_status()
